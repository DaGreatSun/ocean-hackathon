    ...
    routes.map((route) => {
      ...
      route.views.map((view) => {
        if ("functions" in view) {
          let found = view.functions.every((func) =>
            userObject.api.includes(func),
          );
          if (found) {
            viewArr.push(view);
          }
      ...
      if (prop.collapse) {
        ...
        return (
          <div className="group ">
            ...
            >
              ...
              <Collapse.Content>
                {/* {prop.views.map((view, idx) => (
                  ...
                  >
                    <BiSubdirectoryRight className="text-2xl ml-3" />
                    <span className="ml-2 text-sm">{view.name}</span>
                  </div>
                ))} */}
                <>
                  {prop.views.map((view, idx) => {
            ...
            {!isOpen && (
              <div className="invisible fixed left-16 top-0 h-full w-80 -translate-x-3 border-r border-gray-200 bg-white pr-7 shadow-xl transition-all duration-100 group-hover:visible group-hover:translate-x-0 group-hover:opacity-100">
                <h1 className="my-5 ml-6 text-xl font-bold">{prop.name}</h1>
                {prop.views.map((view, idx) => {
                  var stMod = {};
                  stMod[view["state"]] = !state[view.state];
                        ...
                        >
                          ...
                          <Collapse.Content>
                            {view.modules.map((mod) => (
                              ...
                              >
                                <IoMdArrowDropright />
                                <span className="ml-3">{mod.name}</span>
                              </div>
                            ))}
                          </Collapse.Content>
                        </Collapse>
        ...
        return (
          ...
          >
            <div className={`flex h-full items-center rounded-lg `}>
              ...
              >
                <span className="h-[18px] w-[18px]">{prop.icon}</span>
                <p className={`${isOpen ? "ml-4 w-40" : "hidden w-0"}`}>
                  {prop.name}
                </p>
                {!isOpen && (
